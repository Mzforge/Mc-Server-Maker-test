...
function managedProperties(rec) {
  return {
    ...
    // IMPORTANT: MZForge's cracked/official switch is endpoint policy, not
    // Paper's own online-mode. With the official Connect plugin Minekube
    // requires the normal Paper login path and injects Connect sessions, so
    // modern Paper stays online-mode=true while allow-offline-mode-players
    // controls whether cracked Java may enter. Gate backends remain offline.
...
// One connector per endpoint. Minekube documents Connect -> Paper as the
// simplest path for a single modern Paper server. Keep Gate only where the
// Paper plugin cannot run (Vanilla/Fabric/older Java runtimes).
function effectiveConnector(rec) {
  return rec?.loader === "paper" && javaMajor(rec?.mc_version || "") >= 17 ? "plugin" : "gate";
}
...
async function handleAllocate(request, env, url) {
  ...
  const rec = {
    ...
    view_distance: 10, simulation_distance: 10, connect_token: "", connector: loader === "paper" && javaMajor(mcVersion) >= 17 ? "plugin" : "gate",
